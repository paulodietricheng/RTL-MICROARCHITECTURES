...
This design works by comparing only the possible candidates to max in that window. It does it by comparing new inputs to older data in the deque. When the data is older than the duration of the window or smaller 
in magnitude when compared to the new data, this older data is discarded. 

From this experiment, some conclusions were observed. 
The Software Like design performed best in high frequencies and smaller window sizes, because it is simply a purely combinational chain of N-1 muxes. This simple architecture is implemented by using LUTs, which are easy and cheap for FPGAs.

The tree design and the monotonic deque however, are routing heavy, introducing a large routing delay. This was catrastrophic in high frequencies. But, when the size of the window is very large and the frequency is not
so high, the monotonic deque and the tree based design were proven to outperform the simple design. 
